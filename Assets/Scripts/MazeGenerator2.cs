...
public class MazeGenerator : MonoBehaviour
{
    ...
    private List<Vector3> occupiedSkullPos = new List<Vector3>();
    ...
    void GenerateMaze()
    {
        ...
        playerSpawnPoint.transform.position = new Vector3(position.x * 2 * wallSeparation, wallHeight / 8, position.y * 2 * wallSeparation); // wallHeight / 2 è stato cambiato a / 8
       ...
       */
        ...
        for (int i = 0; i < height; i++)
        {
            for (int j = 0; j < width; j++)
            {
                ...
                wallPrefab.layer = LayerMask.NameToLayer("Wall"); // booooooh da rivedere e forse mettere nel prefab
            }
        }
        ...
        // OLD ONE Instantiate(enemy, new Vector3(Random.Range(0, height) * 2 * wallSeparation, wallHeight / 2, Random.Range(0, width) * 2 * wallSeparation), Quaternion.identity);

        // Instantiates spawn points for skulls
        for (int i = 0; i < spawnPointNumber; i++) {
            Vector3 randomPos = GetSkullPosition();

            if(isValidPos(randomPos)) {
                Instantiate(spawnPointPrefab, randomPos, Quaternion.identity);
                occupiedSkullPos.Add(randomPos);
            } else {
                Debug.Log("Posizione occupata... ricalcolo");
                i--;
                continue;
            }
        ...
        //enemySpawnPoint = Instantiate(enemySpawnPoint, new Vector3(Random.Range(0, width) * 2 * wallSeparation, 1.70f, Random.Range(0, width) * 2 * wallSeparation), Quaternion.identity);
        //enemySpawnPoint.gameObject.tag = "EnemySpawnPoint";
        Instantiate(enemy, new Vector3(Random.Range(0, width) * 2 * wallSeparation, 1.70f, Random.Range(0, width) * 2 * wallSeparation), Quaternion.identity);

        //player.transform.position = startPrefab.transform.position;
        //Vector3 paperSheetPos = startPrefab.transform.position + startPrefab.transform.forward * 2f;
        //paperSheet.transform.position = paperSheetPos;

        Debug.Log("Maze ready");
        mazeReady = true;
    }

    private Vector3 GetSkullPosition() {
        return new Vector3(Random.Range(0, width) * 2 * wallSeparation, 1.70f, Random.Range(0, width) * 2 * wallSeparation);
    }

    private bool isValidPos(Vector3 pos) {
        foreach (Vector3 occupiedSkullPos in occupiedSkullPos) {
            if (Vector3.Distance(occupiedSkullPos, pos) < wallSeparation) {
                return false;
            }
        }
